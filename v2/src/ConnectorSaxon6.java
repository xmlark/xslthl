...
public class ConnectorSaxon6 {
	...
	private static void blockToSaxon6Node(Block b, Builder builder, NamePool pool) throws Exception {
		if (b.isStyled()) {
			AttributeCollection emptyAtts = new AttributeCollection(pool);
			int elemId = pool.allocate("", "", ((StyledBlock)b).getStyle());
	...
	protected static NodeEnumeration highlight(Context c, String source, MainHighlighter hl) throws Exception {
		...
		for (Block b : l) {
			blockToSaxon6Node(b, builder, pool);
		}
	...
	public static NodeEnumeration highlight(Context context, String hlCode, String source) throws Exception {
		try {
			...
			return highlight(context, source, hl);
		}
		catch (Exception e) {
			e.printStackTrace();
			return null;
